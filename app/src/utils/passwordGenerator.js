// Password Generator utility for React Native
...
export async function generatePassword(options = {}) {
  ...
  let password = '';
  const array = await getRandomValues(length);
...
export async function generatePassphrase(wordCount = 4) {
  const words = [
  ...
  const selected = [];
  const array = await getRandomValues(wordCount);
